/*
Dr. Sanjay R Kharche.
Tony Workman (Glasgow) AF and BB data
30 November, 2006.

Single file openmp program to simulate AF in the 3D human atrium.
Geomtry received from KTH, Germany (Gunnar Seemann).
*/
...
int main()
{ 
    FILE *out,*geometry,*neighbours,*cont_params,*final_dump,*timedata;
    
    double t = 0.0;
    float start, end;
    double S1S2 = 0.0;
	double udx2,udy2,udz2,sum,distance,ecg;
    ...
    if(AF==0){ // see the Tony 3D code for AF = 0 S1S2.
    S1S2 = 600.0;
    }
    
    if(AF==1){ // using Bosch data S1S2 = 325
 //   S1S2 = 325.0;  // non-trapping parameter
      S1S2 = 450.0;
    }
...
if(counter1!=counter){
	printf("Read in neighbours. Take a look at crn3d_improved_omp_cut_wave_bosch.c program\n");
	exit(EXIT_FAILURE);
}
...
if(first_or_continue==1){ // read in all the data here
	...
	fscanf(cont_params,"%lf %lf %d %d",&start_t,&writetime,&filecounter,&cutted);
	fclose(cont_params);
...
for (t=start_t;t<=start_t+1200.0;t=t+dt)
{
...
fprintf(cont_params,"%f %f %d %d",t,writetime,filecounter,cutted);
fclose(cont_params);

/*
the state array: size of which is counter*24: checkpointing
*/
...
//	ik = ikr + iks;  
    ...
    if(AF==1)
    	icaL = 0.2697*Cm*gcaL*d*f*fca*(V-ErL);
...
if(AF==0) 	  
 	tau = 2.0;  
if(AF==1) 	  
 	tau = 1.62*2.0;    // this is the fast inactivation of Ical - the other one, i.e. f is about 200 times slower
...
//   	vshift = -10;
      ...
      if(AF==1)
   	inf = 1/(1+exp(-(V+20.47+16.0)/17.54));  // ito activation
