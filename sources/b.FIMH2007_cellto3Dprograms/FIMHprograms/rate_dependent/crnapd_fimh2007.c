...
sprintf(str,"ap_profile%d_%g.dat",AF,BCL);
out_two = fopen(str,"w");
free(str);
...
if ((t>=beats*BCL-200)&&(outputcounter%200==0))
	fprintf(out_two,"%10.10f\t%10.10f\t%10.10f\t%10.10f\n",t,state[0],state[9],BCL);
   
if(eold>resting_at_ap*0.9&&state[0]<=resting_at_ap*0.9){ tnew[apdcounter] = t; apdcounter++;}

if(eold>resting_at_ap*0.5&&state[0]<=resting_at_ap*0.5){ tnew50[apdcounter50] = t; apdcounter50++;}
...
// 	printf("%f \n",(state[0] - eold)/dt);
  ...
  /*END OF MAIN LOOP*/
	...
	str = malloc(32*sizeof(char));
	sprintf(str,"apd90%d_%g.dat",AF,BCL);
	out = fopen(str,"w");  
	free(str);
	...
	for(apdcounter=1;apdcounter<imax;apdcounter++){
	  ...
	  if(apdcounter>=imax-1){
	    fprintf(out,"%f\t%f\t%f\t%d\t%f\n",apd[apdcounter],apd50[apdcounter],dis[apdcounter],apdcounter,BCL);
	    fprintf(out,"%f\t%f\t%d\n",vmin,vmax,dvdtmax);
	    }
	}
