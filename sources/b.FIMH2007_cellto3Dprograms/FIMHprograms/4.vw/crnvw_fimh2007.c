/*
Sanjay Kharche
Vulnerability program for  Courtmanche model
...
You will need MATLAB available where ever you work.
*/
...
{ 
    ...
    double s1s1_max, s1s1_min,s1s1_incr;
 
    double t = 0.0;
    double dt= 0.005; /* time step in milliseconds */  
    double BCL = 500.0;
    double state[NODES+1][24],upstroke[NODES+1],downstroke[NODES+1];
    int n,counter;
    double writetime,cv,finaltime;
    double S1S1;
    char *str;
...
t = 0; dt = 0.005; BCL = 500.0; n = 0; counter = 0;
...
for(S1S1=s1s1_max;S1S1>=s1s1_min;S1S1=S1S1-s1s1_incr){ /* change these ranges to get uni-directional conduction */
/* intial conditions. */
    for(n=0;n<=NODES;n++){
...
/*
	...
	sprintf(str,"courtVW%g_%d.dat",S1S1,AF);
	out = fopen(str,"w");
	free(str);
*/

	for(n=0;n<=NODES;n++){
	    upstroke[n] = 0.0; downstroke[n] = 0.0;
	}
	...
	for (t=0;t<=500.0;)
	{
#pragma omp parallel shared (state,new_v,t,dt,S1S1,finaltime,counter) private (n,pulse) default(none)
{
#pragma omp for schedule(static)
	    for(n=1;n<NODES;n++){
		pulse = 0.0;
		if(t>=10&&t<=12&&n<=2&&counter<=1) pulse = 2e3;
		if(finaltime>=S1S1&&finaltime<=(S1S1+2)&&n>(NODES/2-2)&&n<=(NODES/2+2)) pulse = 2e3;
...
/* update VW counter */
	    ...
	    if(state[NODES/2][0]<=-60.0&&new_v[NODES/2]>-60.) counter++;

		/*
		left and right counters
		*/

	if(state[5][0]<=-60.0&&new_v[5]>-60.) leftcount++;
	if(state[NODES-5][0]<=-60.0&&new_v[NODES-5]>-60.) rightcount++;
/*
boundary conditions.
*/
	    new_v[0] = new_v[1];
...
//	fclose(out);
	...
	str = malloc(32*sizeof(char));
	sprintf(str,"vwdata%d.dat",AF);
	leftright = fopen(str,"a+");
	free(str);
	fprintf(leftright,"%f\t%d\t%d\t%d\n",S1S1,leftcount,rightcount,AF);
	fclose(leftright);
