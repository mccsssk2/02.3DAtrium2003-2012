/*
Sanjay Kharche
First run takes less than a couple of hours.

We think theres some point in looking for alternans. So here goes.

Control, AF1 and Af2 of Bosch and Workmann cases.
...
double const gcl = 0.00019; // nS/pF
double const gno3 = 0.00037; // nS/pF
double const eno3 = -45.64; // mV
double const ecl = -50.73; // mV
double const cno3 = 0.87;
double const ccl = 0.94;
double const dno3 = 0.00084; // mV^-1
double const dcl = 0.00025; // mV^-1
 
  
double apd[100],apd50[100],dis[100],told[100],tnew[100],tnew50[100];  
int apdcounter,apdcounter50,imax,someothercounter,numS1;
double eold,stimtime;
double resting_at_ap;

// double S2times[] = {3000,2800,2600,2500,2400,2300,2200,2100,2000,1900,1800,1700,1600,1500,1400,1300,1200,1100,1000,975,950,925,900,875,850,825,800,775,750,725,700,675,650,625,600,575,550,525,500,475,450,425,400,375,350,325,300,275,250,225,200,175,150,125,100,-1,-1};

// double S2times[] = {200,195,190,185,180,175,170,165,160,155,150,145,140,135,130,125,120,115,110,105,100,95,90,85,80,75,-1,-1};

double S2times[1000];

/*
Anion currents and parameters.
*/
...
void courtemach(double dt, double t, double state[24]);  
...
if(AF==0)
for(someothercounter=0;someothercounter<1000;someothercounter++){
S2times[someothercounter] = 300.0 - 1.0*(double)someothercounter;
if(S2times[someothercounter]<=200.0) S2times[someothercounter] = -1.0;
// printf("%f \n",S2times[someothercounter]);
}

if(AF==1)
for(someothercounter=0;someothercounter<1000;someothercounter++){
S2times[someothercounter] = 300.0 - 1.0*(double)someothercounter;
if(S2times[someothercounter]<=100.0) S2times[someothercounter] = -1.0;
// printf("%f \n",S2times[someothercounter]);
}
...
/* start the restitution loop */

do{  

for(apdcounter=0;apdcounter<101;apdcounter++){
apd[apdcounter]=0;
apd50[apdcounter] = 0.0;
dis[apdcounter]=0;
told[apdcounter]=0;
tnew[apdcounter]=0;
tnew50[apdcounter]=0;
...
BCL = S2times[someothercounter];
  
	for (t=0;t<=beats*BCL+S2times[someothercounter]+1000;)  
	{  
	eold = state[0];
	    courtemach(dt, t, state);
		t = t + dt;  

if(stimtime>=0&&stimtime<=dt){ resting_at_ap = state[0];
      told[apdcounter] = t;
      }
 ...
 if(eold>resting_at_ap*0.9&&state[0]<=resting_at_ap*0.9){ tnew[apdcounter] = t; apdcounter++;}
  ...
  if(eold>resting_at_ap*0.5&&state[0]<=resting_at_ap*0.5){ tnew50[apdcounter] = t; apdcounter50++;}
	}   // end of time loop.
  ...
  /*END OF MAIN LOOP*/
	...
	sprintf(str,"alternans%d.dat",AF);
	out = fopen(str,"a+");
	...
	for(apdcounter=1;apdcounter<imax;apdcounter++){
	  ...
	  apd50[apdcounter] = tnew50[apdcounter] - told[apdcounter];
	  dis[apdcounter] = told[apdcounter] - tnew[apdcounter-1];
	 // printf("%f\t%f\t%d\n",apd[apdcounter],dis[apdcounter],apdcounter);
	 // if(apdcounter>=(imax-6)&&apdcounter<(imax-1))  fprintf(out,"%f\t%f\t%f\t%f\t%d\t%d\n",BCL,apd[apdcounter],apd50[apdcounter],dis[apdcounter],apdcounter,apdcounter50);
  if(apdcounter<(imax-1))  fprintf(out,"%f\t%f\t%f\t%f\t%d\t%d\n",BCL,apd[apdcounter],apd50[apdcounter],dis[apdcounter],apdcounter,apdcounter50);
	}
   fclose(out);
  someothercounter++;  
