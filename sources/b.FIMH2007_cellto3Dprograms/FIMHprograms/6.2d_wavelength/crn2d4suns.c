/*
For beta-blocker cases
running on the sun computers

  the output files can now be in ASCII.
  A faster way to run the program is to 
  write your data in binary format.
*/
...
#define HX 0.2 // mm: was 0.1 mm
#define D 0.03125 // mm^2 / ms : FIMH value.
...
double dt=0.1; /* time step in milliseconds: was 0.005 */  
char *str;
int index;
...
// 2.
	...
	if(AF==0)
	if(finaltime>=S1S1&&finaltime<=(S1S1+dt)&&ny>(NODESY/2)) state[n][ny][0] = 100;
...
if(writetime>=10.0){
   ...
   sprintf(str,"court2d%d.dat",filecounter);
   out = fopen(str,"w");
   free(str);
   for(ny=0;ny<=NODESY;ny++){
   for(n=0;n<=NODES;n++){
...
double temporary = 0.0;
	...
	if(AF==2)
	ito = 0.71*Cm*gto*oa*oa*oa*oi*(V-Ek);   
	if(AF==3)
	ito = 0.35*0.71*Cm*gto*oa*oa*oa*oi*(V-Ek);   
   ...
   if(AF==2) // 
	ikur = 1.14*Cm*gkur*ua*ua*ua*ui*(V-Ek); 
   if(AF==3)
	ikur = 1.12*1.14*Cm*gkur*ua*ua*ua*ui*(V-Ek); 
  ...
  if(AF==1) // AF
  	icaL = 0.36*Cm*gcaL*d*f*fca*(V-ErL);  
  if(AF==2) // BB
  	icaL = 1.16*Cm*gcaL*d*f*fca*(V-ErL);  
  if(AF==3) // BB + AF
  	icaL = 0.36*1.16*Cm*gcaL*d*f*fca*(V-ErL);  
  ...
  if(AF==2) // BB
 	ik1 = 0.92*Cm*gk1*(V-Ek)/(1+exp(0.07*(V+80)));
  if(AF==3) // AF + BB
 	ik1 = 0.92*1.9*Cm*gk1*(V-Ek)/(1+exp(0.07*(V+80)));
