...
double apd[100],dis[100],told[100],tnew[100], vpeak[100];  
int apdcounter,imax,someothercounter,numS1;
...
// #if AF==0
// double S2times[] = {385,384,383,382,381,380,-1,-1};


//#if AF==1
// double S2times[] = {295,294,293,292,291,290,-1,-1};


//#if AF==2
// double S2times[] = {260,259,258,257,256,255,-1,-1};
...
double BCL_start, BCL_end, BCL_increment;
...
for(AF=0;AF<3;AF++){
  ...
  if(AF==0)
  for(someothercounter = 0;someothercounter<500;someothercounter++){
  if(400.0 - (double)someothercounter>300.0)
  S2times[someothercounter] = 400.0 - (double)someothercounter;
  else
  S2times[someothercounter] = -1.0;
 ...
 if(AF==1)
  for(someothercounter = 0;someothercounter<500;someothercounter++){
  if(220.0 - (double)someothercounter>90.0)
  S2times[someothercounter] = 220.0 - (double)someothercounter;
  else
  S2times[someothercounter] = -1.0;
...
if(AF==2)
  for(someothercounter = 0;someothercounter<500;someothercounter++){
  if(220.0 - (double)someothercounter>70.0)
  S2times[someothercounter] = 220.0 - (double)someothercounter;
  else
  S2times[someothercounter] = -1.0;
...
for(BCL = BCL_start; BCL>=BCL_end; BCL = BCL - BCL_increment){
...
numS1 = 0;	
 ...
 //  printf("%d \n",numS1);
  ...
  if(eold>resting_at_ap*0.9&&state[0]<=resting_at_ap*0.9){ 
	  tnew[apdcounter] = t; apdcounter++;
  }

  if(state[0]>vpeak[apdcounter]) vpeak[apdcounter] = state[0];
}   // end of time loop.

  /*END OF MAIN LOOP*/
	imax = apdcounter;
	
	out = fopen("courtemanche_erp.dat","a+");  
	if(out==NULL)
	{  
		printf("  can't open %s \n" , "courtemanche_erp.dat");  
		return 2;  
  	}
	...
	printf("%f \t %f \t %f \t %f \t %d \t %f \t %f\n",BCL,S2times[someothercounter],apd[apdcounter],dis[apdcounter],apdcounter,vpeak[apdcounter],vpeak[apdcounter]/vpeak[2]);
	}
	fclose(out);
if(vpeak[imax-1]/vpeak[2]<=0.8) break;	
 someothercounter++;
...
sprintf(str,"crnerp%d.dat",AF);
erp = fopen(str,"a+");
free(str);
fprintf(erp,"%f %f %f %f\n",BCL,S2times[someothercounter],apd[imax-1],vpeak[imax-1]/vpeak[2]);
fclose(erp);
} // end of BCL loop.
