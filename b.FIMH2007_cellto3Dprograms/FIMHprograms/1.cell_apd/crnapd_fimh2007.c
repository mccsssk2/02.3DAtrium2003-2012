...
free(str);
	...
	for (t=0;t<=BCL+S2times[someothercounter]+1000;)  
	{  
	eold = state[0];
 ...
 /* corrections added: all from the powerpoint data v*/
  ...
  /* compute ibna background current */  
    ...
    {  
  	...
  	dvdt = -(ina+icaL+icap+ik1+ito+ikur+ikr+iks+ibna+ibk+ibca+inak+inaca + 0.0*pulse(stimtime))/Cm;
  	
  	something = fopen("sim.dat","a+");
  
	fprintf(something,"%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t %10.10f\t%10.10f\t%10.10\t %10.10f\t %10.10f\t %10.10f\t %10.10f\n",t,ina,icaL,state[0],state[9],-dvdt*Cm,ik1,ito,ikur,inaca,inak,iks,ikr);
