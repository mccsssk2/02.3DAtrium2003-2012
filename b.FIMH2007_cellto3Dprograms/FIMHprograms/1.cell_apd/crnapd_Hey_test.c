...
if(eold>resting_at_ap*0.2&&state[0]<=resting_at_ap*0.2){ tnew[apdcounter] = t; apdcounter++;}
...
{  
 ...
 /* corrections added: all from the powerpoint data v*/
	...
	ito = Cm*(1.0 - 0.248)*gto*oa*oa*oa*oi*(V-Ek);   


	/* compute ultra-rapid potassium current */  

  
	gkur = 0.9*(0.005+0.05/(1+exp((V-15)/-13)));  
	...
	fca = inf + (fca-inf)*exp(-dt/tau);   
 	...
 	ik1 = 1.1*Cm*gk1*(V-Ek)/(1+exp(0.07*(V+80)));   
