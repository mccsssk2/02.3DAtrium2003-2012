...
double diffusion[NODES+1][NODESY+1];
...
// 1. pulse is going from left to right.
 ...
 for(n=1;n<NODES;n++)
 {
  new_v[n][ny] = state[n][ny][0]+dt*(
	  
	  diffusion[n+1][ny]*(state[n+1][ny][0] - state[n][ny][0])/(HX*HX)+
...
#pragma omp parallel shared (diffusion,state,new_v,pulse,t,dt) private(n,ny) default(none)
{
...
diffusion[n+1][ny]*(state[n+1][ny][0] - state[n][ny][0])/(HX*HX)+
diffusion[n-1][ny]*(state[n-1][ny][0] - state[n][ny][0])/(HX*HX)+
diffusion[n][ny+1]*(state[n][ny+1][0] - state[n][ny][0])/(HX*HX)+ 
diffusion[n][ny-1]*(state[n][ny-1][0] - state[n][ny][0])/(HX*HX)
- courtemach(dt,t,state,n,ny)+ pulse[n][ny]);
}
