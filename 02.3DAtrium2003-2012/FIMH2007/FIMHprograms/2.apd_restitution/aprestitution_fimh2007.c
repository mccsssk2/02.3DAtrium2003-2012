...
S2times[someothercounter] = 280.0 - (double)someothercounter;
if(S2times[someothercounter]<=180.0) S2times[someothercounter] = -1.0;
// printf("%f \n",S2times[someothercounter]);
}

if(AF==1)
for(someothercounter=0;someothercounter<1000;someothercounter++){
S2times[someothercounter] = 125.0 - (double)someothercounter;
...
if(AF==2)
for(someothercounter=0;someothercounter<1000;someothercounter++){
S2times[someothercounter] = 100.0 - (double)someothercounter;
if(S2times[someothercounter]<=100.0) S2times[someothercounter] = -1.0;
// printf("%f \n",S2times[someothercounter]);
}
...
if(stimtime>=0&&stimtime<=dt){ resting_at_ap = state[0];
 ...
 if(eold>resting_at_ap*0.9&&state[0]<=resting_at_ap*0.9){ tnew[apdcounter] = t; apdcounter++;}
  ...
  /*END OF MAIN LOOP*/
	...
	sprintf(str,"ap_restitution%d.dat",AF);
	out = fopen(str,"a+");

	if(out==NULL)
	{  
		printf("  can't open %s \n" , "courtemanches1s2.dat");  
		return 0;  
  	}
	...
	for(apdcounter=1;apdcounter<imax;apdcounter++){
	 ...
	 // printf("%f\t%f\t%d\n",apd[apdcounter],dis[apdcounter],apdcounter);
	  if(apdcounter>=imax-1)  fprintf(out,"%f\t%f\t%d\n",dis[apdcounter],apd[apdcounter],apdcounter);
	}
   fclose(out);
  someothercounter++;  
